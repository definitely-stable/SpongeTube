...
internal const val AUTHORITY = "fixture-f1"
internal const val MANIFEST_KEY = "manifest.mpd"

/** Builds the F1 playback plan from the shared FetchUnit catalog. */
@OptIn(SpongeBridgeApi::class)
internal class F1PlanFactory(
    private val fixture: F1FixtureAssets,
    val catalog: F1Catalog,
) {
    fun plan(splitExtentIds: Set<String> = emptySet(), parts: Int = 3): PlaybackPlan {
        val mpdFact = fixture.resourceFacts.getValue(F1FixtureCatalog.MANIFEST_PATH)
        ...
        for (unit in catalog.units) {
            ...
            resources += ExtentPlaybackResource(unit.resourceName, unit.length, units)
        }
        return PlaybackPlan(F1FixtureCatalog.ASSET, resources)
    }

    /**
     * Ranged FetchUnits for one segment. Partial units claim no media interval
     * (conservative coverage); they exist to exercise Range fetch and reads
     * across extent boundaries inside one open.
     */
    private fun split(unit: F1FetchUnit, parts: Int): List<PlaybackFetchUnit> {
        val bytes = fixture.verifiedBytes(unit.resourcePath)
        val boundaries = (0..parts).map { index -> unit.length * index / parts }
        return boundaries.zipWithNext().map { (start, end) ->
            PlaybackFetchUnit(
                fetchKey = FetchKey("${unit.fetchKeyValue}#bytes=$start-${end - 1}"),
                extentSpec = ExtentSpec(
                    mediaAssetId = F1FixtureCatalog.ASSET,
                    extentId = ExtentId("${unit.extentId}@$start-$end"),
                    ...
                    dependencyExtentIds = unit.dependencyExtentIds.map(::ExtentId),
                    expectedLength = end - start,
                    expectedSha256 = Sha256Digest(
                        sha256Hex(bytes, start.toInt(), (end - start).toInt()),
                    ),
                ),
                resourceStart = start,
                transportKey = unit.resourcePath,
