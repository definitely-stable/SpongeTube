...
import io.github.definitelystable.spongetube.core.engine.FetchAttemptExecutor
 ...
 */
internal class ScriptedOrigin : FetchAttemptExecutor {
...
/** Records every requested backoff delay without sleeping. */
internal class RecordingSleeper : RecoverySleeper {
...
/**
 * Deterministic test policy (M2-C item 79): REMOTE_ATTEMPT = 4 and a backoff
 * whose windows are 100, 200, 400 ms; with [FULL_WINDOW_JITTER] the delays
 * are exactly those windows.
 */
internal val TEST_POLICY = RecoveryPolicy(
    budget = RecoveryBudgetPolicy(
        policyId = "sponge-recovery-test-v1",
...
internal class RecoveryHarness(
    ...
    val sleeper: RecoverySleeper = RecordingSleeper(),
